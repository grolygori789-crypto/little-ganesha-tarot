(() => {
  ...
  if (!ENGINE || !CONTENT || !NARRATIVE || !READING_EXPORT || !THREE_EXPORT || !DECK_RITUAL) throw new Error('Three-card UI requires Reading Engine, content, narrative, Deck Ritual, and export modules.');
  ...
  const COPY = {
    en: {
      ...
      readAgain:'Read Three Cards Again', back:'Back to Home', upright:'UPRIGHT', cardAlt:'Tarot card: ',
    ...
    th: {
      ...
      readAgain:'เปิดไพ่สามใบอีกครั้ง', back:'กลับหน้าหลัก', upright:'ไพ่ตั้งตรง', cardAlt:'ไพ่ทาโรต์: ',
  ...
  let session=null, candidates=[], selectedIndices=[], selectedCards=[], reading=null, view='intro', timer=null, previousFocus=null, exportBusy=false, lifecycle=0, deckRitual=null;
  ...
  async function revealReading(){if(!session||session.state!=='selected'||selectedCards.length!==3)return;const token=lifecycle;primary.disabled=true;status.textContent=t('loading');session.beginReveal();reading=NARRATIVE.compose(selectedCards,lang());renderText();revealed.replaceChildren();selectedCards.forEach((card,index)=>{const figure=document.createElement('figure');figure.className='three-result-card';figure.innerHTML=`<span class="three-result-card__position">${reading.positions[index].label}</span><div class="three-result-card__art"><img src="${card.image}" alt="${t('cardAlt')}${card.title[lang()]}" decoding="async"></div><figcaption><strong>${card.title[lang()]}</strong>${lang()==='th'?`<small>${card.title.en}</small>`:''}<p>${reading.positions[index].text}</p></figcaption>`;revealed.appendChild(figure);});
    ...
    if(token!==lifecycle||shell.hidden)return;selectedCards.forEach((card,index)=>{const slot=rail.querySelectorAll('.three-slot')[index];const img=slot.querySelector('img');img.src=card.image;img.alt=`${t('cardAlt')}${card.title[lang()]}`;$('threeSlotName'+index).textContent=card.title[lang()];});session.markRevealed();session.markInterpreted();view='revealed';interpretation.hidden=false;saveShare.hidden=false;home.hidden=false;actions.hidden=false;primary.disabled=false;shell.classList.add('is-revealed');updateCopy();emit('reading-reveal',{cardIds:selectedCards.map(c=>c.id),trajectory:reading.trajectory});after(80,()=>interpretation.scrollIntoView({behavior:reduced()?'auto':'smooth',block:'nearest'}));}
  ...
  function readAgain(){if(session?.state==='interpreted')session.complete();emit('read-again');reset();session=ENGINE.createSession('three');after(50,()=>primary.focus({preventScroll:true}));}
  function primaryAction(){if(view==='intro')start();else if(view==='selected')revealReading();else if(view==='revealed')readAgain();}
  function open(){if(!shell.hidden)return;previousFocus=document.activeElement;reset();session=ENGINE.createSession('three');setMainInert(true);document.body.classList.add('is-reading-open');shell.hidden=false;requestAnimationFrame(()=>shell.classList.add('is-active'));emit('reading-open');after(30,()=>primary.focus({preventScroll:true}));}

(() => {
  ...
  if(!ENGINE||!CONTENT||!NARRATIVE) throw new Error('golden focus storage requires Reading Engine, tarot content, and narrative.');
  const STORAGE_KEY='lgt.reading.golden.v2';
  const LEGACY_KEY='lgt.reading.golden.v1';
  const SCHEMA_VERSION=2;
  const ORIENTATION='upright';
  const FOCUS_IDS=Object.freeze(Object.keys(NARRATIVE.focuses));
  function emptyBox(localDate=ENGINE.localDateISO()){return {schemaVersion:SCHEMA_VERSION,contentVersion:CONTENT.version,spreadId:'golden',localDate,readings:{}};}
  function validReading(record,localDate=ENGINE.localDateISO()){return Boolean(record&&record.contentVersion===CONTENT.version&&record.spreadId==='golden'&&record.localDate===localDate&&typeof record.focusId==='string'&&NARRATIVE.focuses[record.focusId]&&Array.isArray(record.cards)&&record.cards.length===3&&record.cards.every(e=>e?.orientation===ORIENTATION&&CONTENT.getCard(e.cardId))&&new Set(record.cards.map(e=>e.cardId)).size===3);}
  function readBox(localDate=ENGINE.localDateISO()){
    try{const raw=localStorage.getItem(STORAGE_KEY);if(!raw)return emptyBox(localDate);const box=JSON.parse(raw);if(box?.schemaVersion!==SCHEMA_VERSION||box?.contentVersion!==CONTENT.version||box?.spreadId!=='golden'||box?.localDate!==localDate||!box.readings||typeof box.readings!=='object')return emptyBox(localDate);const clean=emptyBox(localDate);for(const id of FOCUS_IDS){const r=box.readings[id];if(validReading(r,localDate))clean.readings[id]={...r,cards:r.cards.map(c=>({...c}))};}return clean;}catch(_){return emptyBox(localDate);}
  }
  ...
  function migrateLegacy(localDate=ENGINE.localDateISO()){
    try{if(localStorage.getItem(STORAGE_KEY))return;const raw=localStorage.getItem(LEGACY_KEY);if(!raw)return;const old=JSON.parse(raw);if(!validReading(old,localDate))return;const focusId=old.focusId;const box=emptyBox(localDate);box.readings[focusId]={...old,focusId};writeBox(box);}catch(_){}
  }
  ...
  window.LGTGoldenStorage=Object.freeze({key:STORAGE_KEY,schemaVersion:SCHEMA_VERSION,get,getAll,count,save,clearExpired});
})();

(() => {
  ...
  const VIEWER_ID = 'goldenCardViewer';
  const REVEALED_ID = 'goldenRevealedCards';
  const SHELL_ID = 'goldenReadingView';
  ...
  function createViewer() {
    ...
    modal.setAttribute('aria-labelledby', 'goldenCardViewerTitle');
    modal.innerHTML = `
      <div class="three-card-viewer__panel" role="document">
        ...
        <div class="three-card-viewer__caption">
          <strong id="goldenCardViewerTitle"></strong>
          <small></small>
        </div>
      </div>`;
    ...
    title = modal.querySelector('#goldenCardViewerTitle');
  ...
  function openViewer(art) {
    ...
    inertTargets = [
      ...
      document.getElementById('goldenScroll'),
      document.getElementById('miniPlayer')
    ].filter(Boolean);
    inertTargets.forEach((target) => setInert(target, true));

    viewer.hidden = false;
    document.body.classList.add('is-golden-card-viewer-open');
  ...
  function closeViewer() {
    ...
    document.body.classList.remove('is-golden-card-viewer-open');
  ...
  window.LGTGoldenCardViewer = Object.freeze({ close: closeViewer });
})();
